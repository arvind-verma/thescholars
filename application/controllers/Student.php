...
//        $page_data['tasks'] = $this->db->get_where('tasks', array("teacher_id" => $this->session->userdata("teacher_id")))->result_array();
        $page_data['tasks'] = $this->db->query("select tasks.*,subject.name from tasks inner join subject on tasks.subject_id = subject.subject_id where tasks.class_id='" . $student_profile["class_id"] . "' and tasks.section_id='" . $student_profile["section_id"] . "' order by tasks.task_id desc")->result_array();
