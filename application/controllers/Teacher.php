...
class Teacher extends CI_Controller {
    ...
    function student($param1 = '', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'createadd') {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(base_url() . 'teacher/student_edit/' . $param2, 'refresh');
        }
        if ($param1 == 'editadd') {
            ...
            redirect(base_url() . 'teacher/student_edit/' . $param3, 'refresh');
        }
        if ($param1 == 'updateadd') {
        ...
        if ($param2 == 'do_update') {
            ...
            if (!empty($_FILES['aadhar_card_file']['name']))
                move_uploaded_file($_FILES['aadhar_card_file']['tmp_name'], 'uploads/aadhar_card/' . $param3 . '.jpg');

            if (!empty($_FILES['medical_certificate']['name']))
                move_uploaded_file($_FILES['medical_certificate']['tmp_name'], 'uploads/medical_certificate/' . $param3 . '.jpg');

            if (!empty($_FILES['birth_certificate']['name']))
                move_uploaded_file($_FILES['birth_certificate']['tmp_name'], 'uploads/birth_certificate/' . $param3 . '.jpg');
    ...
    if ($param1 == 'delete') {
        ...
        redirect(base_url() . 'teacher/parent/', 'refresh');
    }
...
function tasks($param1 = '', $param2 = '') {
    if ($param1 == 'create') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
        redirect(base_url() . 'teacher/tasks/', 'refresh');
    }
    if ($param1 == 'do_update') {
        $data['task_name'] = $this->input->post('task_name');
...
//        $page_data['tasks'] = $this->db->get_where('tasks', array("teacher_id" => $this->session->userdata("teacher_id")))->result_array();
    $page_data['tasks'] = $this->db->query("select tasks.*,subject.name from tasks inner join subject on tasks.subject_id = subject.subject_id where tasks.teacher_id='" . $this->session->userdata("teacher_id") . "' order by tasks.task_id desc")->result_array();
