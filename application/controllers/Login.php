...
class Login extends CI_Controller {
    ...
    function ajax_login() {
        ...
        if ($login_status == 'success') {
            $response['redirect_url'] = base_url() . $this->session->userdata('login_type');
        }
    ...
    function validate_login($email = '', $password = '') {
        ...
        $credential1 = array('admission_no' => $email, 'password' => $password);
        ...
        $query = $this->db->get_where('student', $credential1);
        if ($query->num_rows() > 0) {
            $row = $query->row();
